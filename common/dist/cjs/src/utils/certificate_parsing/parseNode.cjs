...
// src/utils/certificate_parsing/parseNode.ts
var parseNode_exports = {};
__export(parseNode_exports, {
  addOpenSslInfo: () => addOpenSslInfo,
  parseCertificate: () => parseCertificate
});
module.exports = __toCommonJS(parseNode_exports);
...
${pem}
-----END CERTIFICATE-----`;
  (0, import_fs.writeFileSync)(tempCertPath, formattedPem);
  try {
    const openSslOutput = (0, import_child_process.execSync)(`openssl x509 -in ${tempCertPath} -text -noout`).toString();
    ...
    try {
      (0, import_fs.unlinkSync)(tempCertPath);
    } catch (e) {
    }
  }
...
function initElliptic() {
  const curves = import_elliptic.default.curves;
  const PresetCurve = import_elliptic.default.curves.PresetCurve;
  ...
  return import_elliptic.default;
}
...
var getSubjectKeyIdentifier = (cert) => {
  ...
  if (subjectKeyIdentifier) {
    ...
    const hash2 = import_js_sha256.sha256.create();
...
function getParamsRSAPSS(cert) {
  ...
  const rsaPublicKey = new import_pkijs.RSAPublicKey({ schema: asn1PublicKey.result });
  ...
  const pssParams = new import_pkijs.RSASSAPSSParams({ schema: sigAlgParams });
...
function getCertificateFromPem(pemContent) {
  ...
  return new import_pkijs.Certificate({ schema: asn1.result });
}
...
//# sourceMappingURL=parseNode.cjs.map