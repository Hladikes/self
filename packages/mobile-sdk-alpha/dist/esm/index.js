...
var OnboardingFlow = ({ external, setDocument, PassportCamera, NFCScanner }) => {
  ...
  const client = useSelfClient();
  const handleMRZDetected = useCallback2(
    async (mrzData2) => {
      try {
...
var SelfMobileSdk = ({ config, adapters = {}, children }) => /* @__PURE__ */ jsx6(SelfClientProvider, { config, adapters, children });
...
var useDocumentManager = (external) => {
  ...
  const hasRegisteredDocuments = useCallback3(() => {
